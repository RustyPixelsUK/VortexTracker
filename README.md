![](/.github/img/Vortex3.png?raw=true)

The best music tracker for 8bit AY/YM chips.

## New in Version 3.0
- C# Port from Delphi
- OpenGL rendering
- OpenAL audio
- Spectrum and VU bars
- 3AY support
- Built-in OrGen Editor
- YM2149 Synth slave support
- Spectrum Next slave support
- Plugins

## Screenshots

![](/.github/img/VortexTracker01.png?raw=true)

![](/.github/img/VortexTracker02.png?raw=true)

![](/.github/img/VortexTracker03.png?raw=true)

![](/.github/img/VortexTracker04.png?raw=true)

## Download

[Download VortexTracker](https://github.com/benbaker76/VortexTracker/releases/latest/download/VortexTracker.zip)

## Credits
- Copyright (c) 2000-2009 S.V. Bulba (Original [Vortex Project Website](https://bulba.untergrund.net/vortex_e.htm))
- Thanks to Roman Scherbakov for idea and graphics
- Thanks to Shiru Otaku for OrGen plug-in
- Thanks to Denis Seleznev for icons
- Thanks to MmcM, nq, bfox, mr287cc, Oisee, Flexx
- (c) Oisee (Siril/4D) 2010
	- Natural 4th table added, control improvement, MIDI
- Ayumi Engine:
	- Author: Peter Sovietov (True Grue)
	- Vortex Tracker adaptation by Ivan Pirog (Flexx)
	- Sound improvements by Artem Vasilev (wbcbz7)
- (c) Ivan Pirog (Flexx/Enhancers) 2017-2021
	- Version 1.5 - 2.6
- (c) Dexus / [Volutar](https://github.com/Volutar) 2022-2026
	- Version 2.6.1
- (c) Ben Baker / [Rusty Pixels](https://www.rustypixels.uk) 2020-2026
	- C# Port: Version 3.0+
- Thanks to Richard Faulkner / [Technoshed Software](https://technoshedsoftware.com) for updates and code fixes
- Thanks to Zingot / [Zingot Games](https://zingot.serv00.net) for the new logo
